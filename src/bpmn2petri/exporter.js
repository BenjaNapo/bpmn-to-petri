...
class Exporter {
    ...
    export(process = null) {
        ...
        if (process === null) this.result = ret;
        else this.pools[process] = ret;
    }
    ...
    exportAll() {
        this.export();
        for (let process in this.petrinet.processes) this.export(process);
        this.exportGraphviz();
    }
    ...
    exportNode(node, type = "place", net, xmlDoc, process = null) {
        ...
        if (
            this.withDecorators &&
            (node instanceof ANDSplit || node instanceof ANDJoin)
        ) {
            ...
            operator.setAttribute(
                "type",
                node instanceof ANDSplit ? "101" : "102"
            );
        ...
        if (
            this.withDecorators &&
            (ogNode instanceof XORJoin ||
                ogNode instanceof XORSplit ||
                node instanceof XORDecoratedSplit ||
                node instanceof XORDecoratedJoin)
        ) {
            let toolspecific = xmlDoc.createElement("toolspecific");
            toolspecific.setAttribute("tool", "WoPeD");
            toolspecific.setAttribute("version", "1.0");
            ...
            let rawId = node.id;
            const idx = rawId.lastIndexOf("_op");
            let operatorId = idx > -1 ? rawId.substring(0, idx) : rawId;
            operator.setAttribute("id", operatorId);

            // WoPeD type:
            // 104 = XOR-split, 105 = XOR-join
            const isSplit =
                ogNode instanceof XORSplit || node instanceof XORDecoratedSplit;

            operator.setAttribute("type", isSplit ? "104" : "105");
    ...
    exportArc(arc, net, xmlDoc) {
        ...
        if (arc.waypoints.length > 0) {
            ...
            arc.waypoints.forEach((waypoint) => {
                ...
                point.setAttribute(
                    "x",
                    waypoint.x * this.scale + this.nodeSize / 2
                );
                point.setAttribute(
                    "y",
                    waypoint.y * this.scale + this.nodeSize / 2
                );
