...
export class PetriNet {
     ...
     */
    addPlace(place) {
        if (!this.places.has(place.id)) this.places.set(place.id, place);
    }
    ...
    addNewPlace(id, name, x, y, petri, tokens = 0) {
        if (this.places.has(id)) return this.places.get(id);
    ...
    addNewTransition(id, name, x, y, petri) {
        if (this.transitions.has(id)) return this.transitions.get(id);
    ...
    addNewArc(source, target) {
        ...
        if (this.arcs.has(arc.id)) return this.arcs.get(arc.id);
    ...
    addEndTransition(transition, process) {
        if (!this.endTransitions[process]) this.endTransitions[process] = [];
    ...
    addToProcess(element, process) {
        if (!this.processes[process]) this.processes[process] = [];
     ...
     */
    fire(transition) {
        if (transition.canFire()) transition.fire();
    }
...
export class Node {
    constructor(id, name, x = 0, y = 0, petri = null, process = null) {
        ...
        if (this.process) this.petri.addToProcess(this, this.process);
    }
...
export class Transition extends Node {
    constructor(id, name, x = 0, y = 0, petri = null, process = null) {
        ...
        if (petri) petri.addTransition(this);
    }
...
export class XORSplit extends Node {
    constructor(
        ...
        if (this.type === "decorated")
            this.ref = new XORDecoratedSplit(id, name, x, y, petri, process);
        else this.ref = new Place(id, name, x, y, petri, process);
    }

    draw(container, scale, size = 50) {
        if (this.type === "decorated") this.ref.draw(container, scale, size);
        else this.ref.draw(container, scale, size);
    }
}
...
export class XORJoin extends Node {
    constructor(
        ...
        if (this.type === "decorated")
            this.ref = new XORDecoratedJoin(id, name, x, y, petri, process);
        else this.ref = new Place(id, name, x, y, petri, process);
    }
...
/**
 * Decorated XOR-split transition (graphical representation).
 *
 * @class XORDecoratedSplit
 * @extends Transition
 */
export class XORDecoratedSplit extends Transition {
    draw(container, scale, size = 50) {
        const div = super.draw(container, scale, size);
...
/**
 * Decorated XOR-join transition (graphical representation).
 *
 * @class XORDecoratedJoin
 * @extends Transition
 */
export class XORDecoratedJoin extends Transition {
    draw(container, scale, size = 50) {
        const div = super.draw(container, scale, size);
...
export class Arc {
    ...
    draw(container, scale, size = 50) {
        ...
        this.drawArc(
            s,
            { x: this.target.getX(), y: this.target.getY() },
            container,
            scale,
            size,
            true
    ...
    drawArc(source, target, container, scale, size = 50, arrow = false) {
        ...
        div.className = "arc";
        let width = Math.sqrt(
            Math.pow(target.y * scale - source.y * scale, 2) +
                Math.pow(target.x * scale - source.x * scale, 2)
        );
